...
- Each image $\mathbf{x}$ of the Broden dataset is fed into the neural network to be explained. All activation maps $\mathbf{A}_k$ for each unit $\mathbf{k}$ are stored during this inference process.
...
- The deeper the activation maps in a neural network, the smaller its size. Applying a bilinear interpolation to all lower-scale activation maps $\mathbf{A}_k(x)$ allows scaling of all activation maps to the same size as the original input image. The scaled activation maps are called $\mathbf{S}_k(x)$.
- The binarized activation maps $\mathbf{M}_k(x)$ are the result of applying the threshold $\mathbf{T}_k$ to the scaled activation maps $\mathbf{S}_k(x)$ like this: $\mathbf{M}_k(x) = \mathbf{S}_k(x) >= \mathbf{T}_k(x)$. This activation masks $\mathbf{M}_k(x)$ highlights the pixels space, which is responsible for maximizing the activation for a unit $k$ given an image $x$.

\noindent
**Align activation maps with concepts**
In the final step, the activation maps and the concepts are aligned to understand which activation maps correspond to which concepts. To find the concepts each node is looking for, the activation masks $\mathbf{M}_k$ and the human-labeled concept masks $\mathbf{L}_c$ are compared. The intersection over union (IoU) score is used as a measure of similarity.
...
This IoU score represents if a unit $k$ gets excited about a concept $\mathbf{c}$. If the IoU score exceeds a threshold to be defined, the neuron is considered a detector of concept $\mathbf{c}$. It is important to note that one unit could detect several concepts. The final findings to be highlighted in this paper are:
