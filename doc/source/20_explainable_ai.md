...
- The deeper the activation maps in a neural network, the smaller its size. Applying a bilinear interpolation to all lower-scale activation maps $\boldsymbol{A}_k(x)$ allows scaling of all activation maps to the same size as the original input image. The scaled activation maps are called $\boldsymbol{S}_k(x)$.
- The binarized activation maps $\boldsymbol{M}_k(x)$ are the result of applying the threshold $\boldsymbol{T}_k$ to the scaled activation maps $\boldsymbol{S}_k(x)$ like this: $\boldsymbol{M}_k(x) = \boldsymbol{S}_k(x) >= \boldsymbol{T}_k(x)$. This activation masks $\boldsymbol{M}_k(x)$ highlights the pixels space, which is responsible for maximizing the activation for a unit $k$ given an image $x$.
