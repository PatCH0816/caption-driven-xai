...
CLIP solves three common problems in deep learning applications in the research are of machine vision:

\noindent
**Expensive datasets** 
The famouse ImageNet dataset used 49'000 workers from 167 countries to put 15 million images into 22k categories from 2007-2010. CLIP uses text-image-pairs, which are freely available on the internet already, which is obviously a huge advantage regarding reduced costs and less time needed to generate a dataset. [@imagenet_labels_generation]

\noindent
**Transfer learning** 
Transfer learning requires a pre-trained model, which needs to be finetuned on a downstream task in a N-shot setting. Due to CLIP's zero-shot capability, there's no need for finetuning in general. All we need to do is to provide CLIP the text-prompts for a new downstream task and CLIP will assess the alignment of the concepts in the images and the texts. [@clip_blog]

\noindent
**Poor real-world performance** 
Deep learning models are performing very well on benchmark tasks like the ImageNet challenge. Once deployed into the real world, the performance is often not as good as expected, because the models optimized for the benchmark. This effect is due a covariate shift between the benchmark dataset and the real world data. CLIP's performance on the benchmark dataset is typically much more representative, because CLIP isn't finetuned on the benchmark dataset. [@clip_blog]